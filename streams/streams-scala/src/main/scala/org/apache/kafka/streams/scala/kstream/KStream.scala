...
class KStream[K, V](val inner: KStreamJ[K, V]) {
   ...
   * Transform each record of the input stream into zero or more records in the output stream (both key and value type
   * can be altered arbitrarily).
   * A `Transformer` is applied to each input record and computes zero or more output records. In order to assign a state, 
   * the state must be created and registered beforehand via stores added via `addStateStore` or `addGlobalStore` before 
   * they can be connected to the `Transformer` 
   *
   * @param transformer         an instance of `Transformer` that transforms key/value to a tuple (key, value)
      ...
      override def get(): Transformer[K, V, KeyValue[K1, V1]] = {
        new Transformer[K, V, KeyValue[K1, V1]] {
          ...
          override def punctuate(timestamp: Long): KeyValue[K1, V1] = {
            transformer.punctuate(timestamp) match {
